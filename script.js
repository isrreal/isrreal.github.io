...
const englishPages = {
  ...
  'face-clock': {
    ...
    content: [
      ...
      ['#maquina-estados .case-prose p:nth-child(1)', 'Each moment of the interaction is represented by an explicit state. This prevents the interface from skipping stages, running analyses without a face present, or triggering duplicate records.'],
      ['#maquina-estados .case-prose p:nth-child(2)', 'After a failure, the kiosk neither ends the session nor sends empty requests: it returns to detection and waits for a face before trying again. This model makes behavior predictable for both users and maintainers.'],
      ['.state-main-flow .state-node:nth-child(1)', 'Idle'],
      ['.state-main-flow .state-node:nth-child(3)', 'Detection'],
      ['.state-main-flow .state-node:nth-child(5)', 'Analysis'],
      ['.state-main-flow .state-node:nth-child(7)', 'Confirmation'],
      ['.state-main-flow .state-node:nth-child(9)', 'Idle'],
      ['.state-retry-flow .state-error', 'Error'],
      ['.state-retry-flow .state-node:nth-child(4)', 'Try again'],
      ['.state-return', 'returns to detection ↖'],
      ...
      ['#engenharia .case-list li:nth-child(1)', '<strong>Protected biometrics:</strong> quality validation, access controls, and careful handling of sensitive data.'],
      ['#engenharia .case-list li:nth-child(2)', '<strong>Consistent authentication:</strong> persistent JWT revocation, IDOR protection, and constant-time secret comparison.'],
      ['#engenharia .case-list li:nth-child(3)', '<strong>Provisioned kiosk:</strong> devices are activated through a dedicated flow; secrets are not embedded in HTML delivered to the browser.'],
      ['#engenharia .case-list li:nth-child(4)', '<strong>Restricted deployment:</strong> the backend is locally accessible only and published over an HTTPS tunnel, which is also required for browser camera and geolocation access.'],
      ['#engenharia .case-list li:nth-child(5)', '<strong>Durable data:</strong> versioned migrations, dedicated volumes, and installation verification from an empty database.'],
    ...
    attributes: [
      ...
      ['.state-machine', 'aria-label', 'Kiosk state machine: idle, detection, analysis, confirmation, and return to idle; on error, a new attempt returns to detection.'],
