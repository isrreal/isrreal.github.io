...
grafos e a entrega solo de um produto de visão computacional em produção
...
\begin{itemize}
    ...
    \item Entreguei uma \textbf{API de busca multimodal top-$k$} em FastAPI
    com representações relativas (RelRep), similaridade BERTScore, fusão
    \textit{early}/\textit{late} e indexação vetorial em FAISS, atingindo
    \textbf{Recall@1 $\approx$ 0,97} no componente textual.
...
\begin{itemize}
    ...
    \item Entreguei \textbf{sozinho} um sistema completo de registro de
    ponto por reconhecimento facial --- modelagem de ML, \textit{backend},
    segurança, banco de dados e \textit{deploy} --- atualmente em operação
    para o cliente.
...
\begin{itemize}
    ...
    sistema MLOps ponta a ponta para previsão de notificações de dengue por
    estado, comparando LSTM e PLE (redução do RMSE de $\sim$98 para
    $\sim$83 na série do Ceará); ETL do SINAN via pySUS, PostgreSQL, MLflow,
